...
```
  ...
  Platform specialists (this repo — H5 + mini games):
    web-h5-specialist           -- Browser TS, src/web/, no wx/tt leakage
    wechat-minigame-specialist  -- WeChat mini game APIs and src/platforms/wechat/
    douyin-minigame-specialist  -- Douyin mini game APIs and src/platforms/douyin/
```
...
| `web-h5-specialist` | (owns web stack; coordinates with platform specialists via `lead-programmer`) |
| `wechat-minigame-specialist` / `douyin-minigame-specialist` | (platform APIs and platform directories only) |
