...
This repo adapts the Claude Code Game Studios pattern for **H5 + WeChat +
Douyin** delivery, with **OpenAI-compatible local LLM** usage documented in
`STUDIO.md`. Engine-specific agents (Godot/Unity/Unreal) are removed from
`.claude/agents/` here; the upstream snapshot remains under
`vendor/claude-game-studios/`. Design theory grounding is unchanged.
...
| Ship browser H5 (TS) | `web-h5-specialist` |
| WeChat mini game APIs / layout | `wechat-minigame-specialist` |
| Douyin mini game APIs / layout | `douyin-minigame-specialist` |
| Local OpenAI-compatible setup | skill `local-llm` or `start-local` |
...
2. **Set up the web stack** — Run `/setup-web` (skill `setup-web`)
   - Configures `STUDIO.md` + `.claude/docs/technical-preferences.md`
   - Add `/setup-wechat-minigame` / `/setup-douyin-minigame` when those targets apply
...
If you already have a game concept and stack choice:

1. **Set up the web surface** — Run `/setup-web` (+ platform skills as needed)
...
### Path C: "I know the game but not the web framework"

1. **Run `/setup-web`** with no arguments — it gathers constraints and recommends a TS web framework.
...
2. **Configure web/platforms if needed** — Run `/setup-web` and mini game skills if dirs are empty
...
```
STUDIO.md                          -- Master config for this fork (read first)
.claude/
  settings.json                    -- Claude Code hooks and project settings
  agents/                          -- Agent definitions (trimmed + platform agents)
  skills/                          -- Slash / command-id workflows (YAML frontmatter)
  hooks/                           -- 8 hook scripts (.sh) wired by settings.json
  rules/                           -- 11 path-specific rule files
  docs/
    quick-start.md                 -- This file
    technical-preferences.md       -- Project-specific standards (populated by /setup-web)
