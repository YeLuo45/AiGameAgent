...
- **Stack recommendation** (Web TS framework + whether WeChat/Douyin ships matter)
  with reasoning based on concept needs, team expertise, and platform targets
...
6. **Suggest next steps** (in this order — this is the professional studio
   ...
   - "Run `/setup-web` to pin the browser stack; add `/setup-wechat-minigame` / `/setup-douyin-minigame` when needed"
...
7. **Output a summary** with the chosen concept's elevator pitch, pillars,
   primary player type, stack recommendation (web + mini games), biggest risk, and file path.
