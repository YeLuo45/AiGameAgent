...
Check:
- **Web stack configured?** Read `.claude/docs/technical-preferences.md`. If the
  web stack still reads `[TO BE CONFIGURED]`, run `setup-web` first.
- **Game concept exists?** Check for `design/gdd/game-concept.md`.
- **Source code exists?** Glob for source under `src/web/`, `src/platforms/`, `packages/shared/` (`*.ts`, `*.js`, etc.).
...
2. Based on their answers, offer two paths:
   ...
   - **Jump to web setup**: If they're confident in their concept, go
     straight to `/setup-web` and write the GDD manually afterward
...
3. If the web stack isn't configured, note that `/setup-web` should come first
4. Show the recommended path:
...
- **User is returning (web stack configured, concept exists)**: Skip onboarding
  ...
  may have a game concept at `design/gdd/game-concept.md`. Want to pick up where
